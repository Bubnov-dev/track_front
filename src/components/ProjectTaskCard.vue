        ...
        <template v-else>
            ...
            <div class="project__additional flex">
                <div class="timer" :class="{'active' : element.id == timer.task_id}">
                    ...
                    <!--                                                    </svg>-->
                    <!--                                                </router-link>-->
                    <div class="timer__time" @click="openTimer(element, this.element.time)">
                        {{ service.formatTime(getTimeOfTask()) }}
                    </div>
...
export default {
    ...
    methods: {
        getTimeOfTask(){
            return this.element.timings.reduce((acc, cur) => acc + cur.time, 0) + (this.element.id === this.timer.task_id? this.timer.actualSec : 0);
        },
        deleteTask() {
        ...
        openTimer() {
            this.$emit('request-open-timer', this.element);
        },
